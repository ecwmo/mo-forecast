...
if [ "${NAMELIST_RUN}" -eq 3 ]; then
	...
	MET_EM_FILES=("$WRF_MAINDIR/WPS_ECMWF/$WPS_NAMELIST_SUFF"/met_em.d0*)
	...
	ln -s "$WRF_MAINDIR/WPS_ECMWF/$WPS_NAMELIST_SUFF"/met_em.d0* .
	...
	NUM_TIMESTEPS=$((WRF_FCST_DAYS * 24 + 1))
	MET_EM_FILES=("$WRF_MAINDIR/WPS_GFS/$WPS_NAMELIST_SUFF"/met_em.d0*)
	if [ ${#MET_EM_FILES[@]} -ne $NUM_TIMESTEPS ]; then
		err_msg="number of met_em files: ${#MET_EM_FILES[@]}, expected: $NUM_TIMESTEPS"
		echo "$err_msg" >>"$ERROR_FILE"
		echo "$err_msg"
	...
	ln -s "$WRF_MAINDIR/WPS_GFS/$WPS_NAMELIST_SUFF"/met_em.d0* .
	for f in met_em.*; do mv -v "$f" "$(echo "$f" | tr ':' '_')"; done
