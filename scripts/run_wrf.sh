...
if echo "$NAMELIST_RUN" | grep -qE "run1|run2"; then
	# GFS input
	MET_EM_FILES=("$WRF_MAINDIR/WPS/$WPS_NAMELIST_SUFF"/gfs/met_em.d0*)
	...
	ln -s "$WRF_MAINDIR/WPS/$WPS_NAMELIST_SUFF"/gfs/met_em.d0* .
...
elif echo "$NAMELIST_RUN" | grep -qE "run3"; then
	# ECMWF input
