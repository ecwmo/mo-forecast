...
def get_vimfc(ds):
    ...
    U = ds['u'].sel(isobaricInhPa=target_levels).sortby("isobaricInhPa", ascending=False)
    V = ds['v'].sel(isobaricInhPa=target_levels).sortby("isobaricInhPa", ascending=False)
    Q = ds['q'].sel(isobaricInhPa=target_levels).sortby("isobaricInhPa", ascending=False)
    ...
    vimfc.attrs["long_name"] = "Vertically Integrated Moisture Flux Convergence (1000–925 hPa)"
...
def shearline_detection(ds_hr, vimfc_ds):
    ...
    for i, lon in enumerate(lons_common):
        valid_mask = (dV_dy[:, i] <= -1e-5) & (vimfc_sub.values[:, i] > 0) & (RV_925_sub.values[:, i] > 0)
    ...
    for cluster in clusters:
        for pt in cluster:
            idx = np.where((shear_points[:, 0] == pt[0]) & (shear_points[:, 1] == pt[1]))[0]
...
def save_shearline_csv(ds, out_dir):
    ...
    for hour in ds.time.to_numpy()[1:]:
        hour_datetime = (ds.sel(time=hour).time + pd.Timedelta(hours=8)).dt.strftime('%d %H').item()

        ds_hr = ds.sel(time=hour, drop=True)
        vimfc_ds = get_vimfc(ds_hr)  
        smoothed_shear_lines = shearline_detection(ds_hr, vimfc_ds)
        if len(smoothed_shear_lines) == 0: pass
        else:
            for i,label in enumerate(list(smoothed_shear_lines.keys())):
                ...
                smoothed_shear_lon[:len(smoothed_shear_lines[label][1])] = smoothed_shear_lines[label][1]
                smoothed_shear_lat[:len(smoothed_shear_lines[label][0])] = smoothed_shear_lines[label][0]

                new_row = pd.DataFrame({"index": np.arange(0,1500,1), 
                                        f'{hour_datetime}_{i}_lon': smoothed_shear_lon,
                                        f'{hour_datetime}_{i}_lat': smoothed_shear_lat}).set_index('index').T
...
def plot_weather_chart(ds, shearline_df, out_dir):
    ...
    for hour in range(1,len(ds.time),1):
        ...
        init_datetime = (ds.isel(time=0).time + pd.Timedelta(hours=8)).dt.strftime('%Y-%m-%d %H').item()
        dates_datetime = (ds.isel(time=hour).time + pd.Timedelta(hours=8)).dt.strftime('%Y-%m-%d %H').item()
        ...
        ### PLOT STREAMLINE
        ax.streamplot(
            ds_925.lon, ds_925.lat, ds_925.u.values, ds_925.v.values,
            color="gray", linewidth=1,
            density=14, arrowsize=2, arrowstyle="->")
        
        ### PLOT SHEARLINE
        if f"{dates_datetime[-5:]}_0_lon" in shearline_df.columns:
            ax.plot(shearline_df[f"{dates_datetime[-5:]}_0_lon"],shearline_df[f"{dates_datetime[-5:]}_0_lat"], 
                linewidth=5, linestyle="-.", color="#3D0087")
        else: pass

        ### PLOT THICKNESS
        th_levels = np.arange(0,7000,10)
        kw_clabels = {'fontsize': fs-6, 'inline': True, 'inline_spacing': 2, 'fmt': '%i',
                    'rightside_up': True, 'use_clabeltext': True}

        th = ax.contour(ds_hr.lon, ds_hr.lat, thickness_1000_850, levels=th_levels, colors="#025BD7",
                        linewidths=1.0, linestyles='--')
        plt.clabel(th, **kw_clabels)

        # PLOT MSLP
        clevmslp = np.arange(800., 1120., 4)
        cs = ax.contour(ds_hr.lon, ds_hr.lat, mslp, clevmslp, colors='k', linewidths=1.25,
                        linestyles='solid')
        plt.clabel(cs, **kw_clabels)
        # Use definition to plot H/L symbols
        plot_maxmin_points(ax, ds_hr.lon[15:-15], ds_hr.lat[15:-15], mslp[15:-15, 15:-15], 'max', 50, symbol='H', color='b')
        plot_maxmin_points(ax, ds_hr.lon[15:-15], ds_hr.lat[15:-15], mslp[15:-15, 15:-15], 'min', 30, symbol='L', color='r')

        elev = [0,1,5,10,50,250,850,800,900,1000,1250,1850,2000]
        elev_ds['hgt'].plot.contourf(
                extend = 'max', levels= elev, alpha=0.8,
        ...
        ax.set_xticklabels([f'{int(x)}$\degree$E' for x in np.arange(110,160, 10)], fontsize = fs-6)
        ax.set_yticks(np.arange(0,30, 5))
        ax.set_yticklabels([f'{int(x)}$\degree$N' for x in np.arange(0,30, 5)], fontsize = fs-6)
        ax.set_ylabel('')
        ax.set_xlabel('')
            
        ax.text(149.8, 2.5, f'{dates_datetime} LST\nSurface Analysis\nGFS INIT {init_datetime} LST', 
                fontweight='bold',
                backgroundcolor='w', fontsize = fs, horizontalalignment='center', 
        ...
        fig.legend(ncol= 3, handles=legend_lines, fontsize = fs, bbox_to_anchor=(0.6, 0.95),)
        ...
        out_file = (
            out_dir
            / f"{file_prfx}-{hour*3}hr_weatherchart_{ts:%Y-%m-%d_%H}PHT.png"
        )
...
def proc(in_dir, out_dir):
    ...
    _out_dir = out_dir / f"shearline/csv"
