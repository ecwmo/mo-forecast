    ...
    for file in filepath:
        ...
        for t in ecmwf.time.values:
            ...
            if hour in np.arange(0,24,3):
                if hour < 10:
                    pyw.cinter('FILE', f'{day}_0{hour}', geo, fields, out_dir)
                else:
                    pyw.cinter('FILE', f'{day}_{hour}', geo, fields, out_dir)
            elif hour in np.arange(24,48,3):
                hr = hour - 24
                if hr < 10:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=1)}_0{hr}', geo, fields, out_dir)
                else:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=1)}_{hr}', geo, fields, out_dir)
            elif hour in np.arange(48,72,3):
                hr = hour - 48
                if hr < 10:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=2)}_0{hr}', geo, fields, out_dir)
                else:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=2)}_{hr}', geo, fields, out_dir)
            elif hour in np.arange(72,96,3):
                hr = hour - 72
                if hr < 10:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=3)}_0{hr}', geo, fields, out_dir)
                else:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=3)}_{hr}', geo, fields, out_dir)
            elif hour in np.arange(96,120,3):
                hr = hour - 96
                if hr < 10:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=4)}_0{hr}', geo, fields, out_dir)
                else:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=4)}_{hr}', geo, fields, out_dir)
            elif hour in np.arange(120,144,3):
                hr = hour - 120
                if hr < 10:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=5)}_0{hr}', geo, fields, out_dir)
                else:
                    pyw.cinter('FILE', f'{day + dt.timedelta(days=5)}_{hr}', geo, fields, out_dir)          
